...
3. 必要に応じて「transportationSearch」「weather」を使い、深い調査や事実確認が必要なときだけサブエージェント（research / enhancement / factcheck）を使うこと。自分のツールで十分に分かることをサブエージェントに重ねて投げないこと。
4. 十分に具体的な場所を集めたら、「finalizeDay」ツールを呼び、見つけた実在の場所を使った完全な PlanDay（startTime 付きで4〜7件の順序立てた項目）を出力して終了すること。スキーマ（各 item の type・title 等）を厳守し、メタ的な文言を入れないこと。ステップが残り少ない場合は、検索を続けずに今ある情報で直ちに finalizeDay を呼ぶこと。
5. ツール呼び出しやサブエージェントの上限を超えると強制停止されるため、効率的に動き、早めに finalize すること。
