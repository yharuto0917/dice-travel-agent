...
3. 1日辺り必ず「generateImage」を正確に2回呼び出し、その日の見どころや食事などの風景画像を生成すること。生成した画像のURLを結果に含めること。
...
5. 十分に具体的な場所を集め、画像生成を2回完了したら、「finalizeDay」ツールを呼び、見つけた実在の場所を使った完全な PlanDay（startTime 付きで4〜7件の順序立てた項目）を出力して終了すること。スキーマ（各 item の type・title 等）を厳守し、メタ的な文言を入れないこと。ステップが残り少ない場合は、検索を続けずに今ある情報で直ちに finalizeDay を呼ぶこと。
